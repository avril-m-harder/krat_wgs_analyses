...
hist(HI.RAND.STATS[,2], xlab = 'total length of overlapped genes', breaks = 15, main = 'gene overlap with randomly selected regions (hi-ROH)',
  ...
  hist(HI.RAND.STATS[,3], xlab = 'total number of genes', breaks = 20, main = 'gene overlap with randomly selected regions (hi-ROH)', xlim = c(min(HI.RAND.STATS[,3]), nrow(hi.roh.gene.overlap)))
    abline(v = nrow(hi.roh.gene.overlap), lty = 2, col = 'red')
dev.off()
...
pdf('../krat_genetics_scripts/figures_output/no_roh_region_randomization_histograms.pdf', width = 6, height = 5)
hist(NO.RAND.STATS[,2], xlab = 'total gene length', breaks = 20, main = 'gene overlap with randomly selected regions (no-ROH)', xlim = c(min(NO.RAND.STATS[,2]), 10e7))
  abline(v = no.roh.gene.len, lty = 2, col = 'red')
  
hist(NO.RAND.STATS[,3], xlab = 'total number of genes', breaks = 20, main = 'gene overlap with randomly selected regions (no-ROH)', xlim = c(min(NO.RAND.STATS[,3]), nrow(no.roh.gene.overlap)))
  abline(v = nrow(no.roh.gene.overlap), lty = 2, col = 'red')
dev.off()
